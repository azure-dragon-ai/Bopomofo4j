...
public class LocalPinyinLibrary implements IPinyinLibrary {
    ...
    final static String PINYIN2CHSC_FILE_NAME = "META-INF/resources/bopomofo/libs/v101/pinyin2chsc.json";
    ...
    public LocalPinyinLibrary init() {
        ...
        {
            URL url = this.getClass().getClassLoader().getResource(PINYIN2CHSC_FILE_NAME);
            try {
                this.pinyin2chsc = JSON.parseList(url);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
    ...
    public List<String> getChineseChars(String w) {
        List<String> chs = this.pinyin2chsc.get(w);
        if (chs == null) {
